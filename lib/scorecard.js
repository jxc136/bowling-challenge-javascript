  ...
  resolveStrike() {
    ...
    this.score.forEach((frame, index) => {
      if (frame['isStrike'] && frame['bonusStatus']) {
        ...
        if (nextFrame) {
          bonus += nextFrame['rollOne'] + nextFrame['rollTwo'];
          // Mark the bonus as resolved for the next frame
          nextFrame['bonusStatus'] = false;
        }
        let nextNextFrame = this.score[index + 2];
        if (nextNextFrame) {
          bonus += nextNextFrame['rollOne'];
          // Mark the bonus as resolved for the next-next frame if it exists
          nextNextFrame['bonusStatus'] = false;
        }
        frame['frameTotal'] += bonus;
